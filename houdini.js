...
(function (root, factory) {
	...
	var collapseContent = function (btn, target, settings) {
		...
		elem.classList.remove(settings.visibleClass);

		// When the transition is complete, hide it
		window.setTimeout(function () {
			elem.classList.remove(settings.expandedClass);
		}, 150);		
	...
	var expandContent = function (btn, target, settings) {
		...
		elem.classList.add(settings.expandedClass); // Make the element active
		elem.style.height = height; // Update the max-height		

		// Once the transition is complete, remove the inline max-height so the content can scale responsively
		window.setTimeout(function () {
			elem.style.height = '';
			elem.classList.add(settings.visibleClass); // Make the element visible
		}, 150);
	...
	var teardown = function (selector, settings) {
		Array.prototype.forEach.call(document.querySelectorAll(selector), (function (content) {
			...
			if (settings.expandedOnInit.indexOf(content.id) < 0) {
				...
				content.classList.remove(settings.visibleClass);
			} else {
				content.classList.add(settings.expandedClass);
				content.classList.add(settings.visibleClass);
			}
